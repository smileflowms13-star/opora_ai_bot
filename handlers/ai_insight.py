...
async def ai_insight_command(message: Message):
    ...
    if not diary_entries:
        await message.answer("Пока недостаточно данных для AI-анализа. Веди дневник хотя бы несколько дней.")
        return
    ...
    if not recent:
        await message.answer("За последние 7 дней нет записей дневника. Попробуй заполнить хотя бы пару раз.")
        return
    ...
    try:
        ...
        if not ai_answer:
            ai_answer = "Не удалось сформировать анализ. Попробуй позже."

    except Exception as e:
        logging.exception("AI insight failed")
        ai_answer = "Произошла ошибка при анализе. Попробуй чуть позже."

    await message.answer(f"📊 AI-анализ твоего дневника за неделю:\n\n{ai_answer}")